...
# REVER O TEMPO DE RESIDÊNCIA DAS RAÌZES FINAS - VARIAR ENTRE 1 mes e 2 anos
    ...
    #                     0.083333, 2)
    rtime = np.random.uniform(0.08333333333333333, 8.3333333333333, r_ceil)
    #rtime = np.random.uniform(0.08333333333333333, 7.0, r_ceil)
    ...
    # 2, 4, r_ceil), 1.0, 150)
    rtime_wood = np.random.uniform(2.0, 150.0, r_ceil)
    while index1 < diffw:
        restime = np.zeros(shape=(3,), dtype=np.float64)
